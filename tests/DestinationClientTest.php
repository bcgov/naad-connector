...
final class DestinationClientTest extends TestCase
{
     ...
     */
    #[Test]
    public function testSendRequestHeadersAreCorrect()
    {
        // Define the expected XML payload
        $xmlPayload = '<xml>test</xml>';


        // Mock the HTTP client to verify the post method is called
        // with the correct URL and body
        $this->mockHttpClient->expects($this->once())
            ->method('post')
            ->with(
                '', // Empty URL (as per the implementation)
                $this->callback(
                    function ($options) use ($xmlPayload) {
        ...
        ->willReturn(new Response(200, $this->expectedHeaders, 'OK'));

        // Instantiate the DestinationClient with the mocked dependencies
        $destinationClient = new DestinationClient(
            $this->mockLogger,
            $this->mockDatabase,
            $this->mockHttpClient
        ...
        // Call the sendRequest method with the sample XML payload
        $result = $destinationClient->sendRequest($xmlPayload);

        // Assert that the response is as expected (status_code, body, and headers)
        $this->assertSame(
            [
            'status_code' => 200,
            'body'        => 'OK',
